...
const ProfileView = () => {
  ...
  useEffect(() => {
    ...
    if (storedUser && storedUser.email) {
      fetch(`https://api.smlnexgenllp.com/api/users/getUserDetails?email=${storedUser.email}`)
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`https://api.smlnexgenllp.com/api/users/${storedUser.id}`, {
        method: "PUT",
        body: formData,
      });
      if (response.ok) {
